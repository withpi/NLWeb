...
class NLWebHandler:
    ...
    def __init__(self, query_params, http_handler):
        ...
        self.required_item_type = get_param(
            query_params, "required_item_type", str, None
        )
        ...
        self.handler_message_id = (
            f"msg_{int(time.time() * 1000)}_{uuid.uuid4().hex[:9]}"
        )
    ...
    def from_message(cls, message, http_handler):
        ...
        if hasattr(message, "metadata") and message.metadata:
            ...
            if "search_all_users" in message.metadata:
                query_params["search_all_users"] = [
                    str(message.metadata["search_all_users"]).lower()
                ]
    ...
    async def runQuery(self):
        logger.info(
            f"Starting query execution for conversation_id: {self.conversation_id}"
        )
        try:
    ...
    async def prepare(self):
        ...
        if requested_tool:
            ...
            self.tool_routing_results = [
                {
                    "tool": type(
                        "Tool", (), {"name": requested_tool, "handler_class": None}
                    )(),
                    "score": 100,
                    "result": {
                        "score": 100,
                        "justification": f"Tool {requested_tool} specified in request",
                    },
                }
            ]
        else:
